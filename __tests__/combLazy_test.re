...
let trimEnd = (p) => keepFirst([p, maybeWhitespace] |> toLazy);
    ...
    () => {
      test(
        ...
        () => {
          let result =
            seq(
              [alt([letters, digit] |> toLazy), alt([letters, digit] |> toLazy)] |> toLazy,
              "abcd123"
            );
      ...
      test(
        "seq",
        () => {
          let result = seq([letter, digit, digit, digit] |> toLazy, "a123**");
          let correctResult =
      ...
      test(
        "keep",
        () => {
          let result = keep([1, 2], [letter, digit, digit, digit] |> toLazy, "a123**");
      ...
      test(
        "keepFirst",
        () => {
          let result = keepFirst([letter, digit, digit, digit] |> toLazy, "a123**");
